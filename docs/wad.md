...
> Atualização da fundação: esta versão histórica deve ser lida junto de [evidências da continuação](#evidencias). O contrato OpenAPI estruturado, o teste HTTP, o exemplo de ambiente e o CI com variáveis fictícias foram entregues posteriormente. O [modelo atualizado](#modelo), as [limitações de integridade](#integridade) e o [guia manual](#acoes-manuais) prevalecem sobre os respectivos itens pendentes descritos abaixo. Não há comprovação de CI remoto, segurança do painel ou homologação.
...
O arquivo local de segredos não integra a documentação. O arquivo .env.example contém placeholders. Este WAD é o único documento Markdown; os antigos guias e o README foram incorporados à seção 10. O OpenAPI em contracts/ é um artefato técnico validado pelo CI.
...
Na Sprint 1, o foco é preparar modelo, repositório, contrato, qualidade e ambientes. A existência das tabelas não significa que a admissão esteja disponível aos assistentes.
...
| RN01 | Raça deve corresponder à espécie | Pendente | FKs separadas não garantem correspondência entre os dois vínculos |
| RN02 | Localização deve pertencer à unidade do animal | Pendente | FKs separadas não garantem correspondência |
...
| Compatibilidade | Contrato acordado com frontend | Markdown preliminar; OpenAPI completo ausente |
...
Sua aplicação foi confirmada na sessão anterior. O README ainda contém a informação histórica de que a inicial não foi aplicada e precisa ser atualizado. A migration já aplicada não deve ser reescrita.

As FKs protegem existência de referências, mas não a consistência cruzada raça/espécie e localização/unidade. O desenho atual guarda vínculos presentes e não conserva histórico de transferências ou readmissões.
...
Os identificadores em inglês acima preservam o rascunho existente; novos nomes em português e eventual alteração de rotas precisam ser acordados com frontend. O contrato ainda não é uma especificação OpenAPI completa e validável.
...
Há uma pendência concreta: o schema requer DATABASE_URL e DIRECT_URL, mas o workflow não define essas variáveis. A validação precisa receber URLs fictícias válidas para etapas sem conexão, sem usar segredos de produção. Não existe etapa de validação OpenAPI.
...
| Contrato OpenAPI completo | Ausente |
| HTTP automatizado e integridade cruzada | Ausentes |
| Cobertura percentual | Sem medição apresentada neste documento |
| Carga e offline | Não implementados/testados |

A presente tarefa é documental: os arquivos foram inspecionados e não houve repetição de testes executáveis nem de operações remotas. Os resultados históricos não são apresentados como nova execução.

## 5.2. Plano de testes pendentes

- Testar HTTP de saúde e erros sanitizados, incluindo exceções inesperadas.
- Validar raça incompatível com espécie e localização incompatível com unidade em ambiente isolado.
- Testar cadastro incompleto, foto ausente, campo desconhecido e atualização concorrente.
- Testar reenvio idempotente e interrupção durante envio de foto.
- Validar instalação e pipeline sem .env local ou credenciais remotas.
...
| Modelo e migrations | Implementados; validação de negócio pendente |
| Conexão Supabase de desenvolvimento | Verificada anteriormente |
| Health e Swagger básico | Implementados |
| Filtro global de erros | Implementado; revisão e testes pendentes |
| Contrato completo acordado com frontend | Pendente |
| CI | Arquivo preparado; variáveis e execução remota pendentes |
| .env.example | Ausente na revisão |
| Teste HTTP automatizado | Pendente |
| Integridade cruzada de relações | Pendente |
| Revisão de segurança Supabase | Pendente |
| Documentação e coerência dos artefatos | Parcial; README desatualizado em migrations |
| Ambientes de homologação e publicação da API | Não verificados/pendentes |
| Protótipos de admissão e busca | Não disponíveis neste repositório |

## 7.3. Próximos passos priorizados

1. Validar com Ampara a ficha, obrigatoriedade, readmissão e classificação operacional.
2. Fechar contrato OpenAPI com frontend, incluindo foto e estados de sincronização.
3. Corrigir configuração de CI e exemplo de ambiente; ampliar testes de saúde e erros.
4. Implementar integridade, autenticação e autorização compatíveis com o acesso escolhido.
5. Entregar admissão com foto, busca e offline no Ciclo 1.
6. Publicar e testar em homologação antes da liberação para uso real.
...
Esta seção preserva os guias anteriormente separados. Em caso de divergência com a descrição histórica acima, prevalece o estado desta seção. O contrato executável foi movido para [contracts/openapi.json](../contracts/openapi.json). Verificação desta organização: oito testes, tipos e lint passaram; o teste HTTP apresentava divergência de formatação do Prettier.

<a id="acoes-manuais"></a>

## Ações manuais — Sprint 1
...
## Comandos executados
npm ci, npm run prisma:generate, npm run openapi:validate, npm run lint, npm run typecheck, npm test, npm run prisma:validate e npm run build: aprovados. Duas suítes e oito testes passaram. URLs de validação apontaram para loopback porta 1; nenhuma consulta remota ou migration foi executada.
npm ci informou oito vulnerabilidades altas nas dependências. Não foi usado audit fix --force; triagem e atualização compatível permanecem pendentes.
## Preparado
...
FKs ainda não impedem incompatibilidade raça/espécie e localização/unidade. Proposta e campos opcionais em integridade-admissao.md. Não há histórico de peso.
...
O schema e as duas migrations aplicadas foram preservados nesta revisão. As FKs garantem existência, mas **não impedem** Animal.breedId apontar para raça de outra speciesId, nem locationId para localização de outra unitId. Não há testes de banco que comprovem essas regras. Isso bloqueia a entrega do cadastro funcional.
A correção proposta é FK composta (breedId, speciesId) → Breed(id, speciesId) e (locationId, unitId) → Location(id, unitId), com índices únicos correspondentes e análise das ações ON DELETE/UPDATE. Antes de nova migration, consultar inconsistências e acordar atualização dos vínculos; não preencher ou apagar registros para corrigir automaticamente. Raça desconhecida permanece NULL. Outra opção é validação transacional no serviço, mas não protegeria escritas fora dele. Nenhuma dessas correções está implementada nesta tarefa.
## Campos provisórios
...
Veja [contrato](../contracts/openapi.json), [modelo](#modelo), [limitações de integridade](#integridade), [segurança](#seguranca) e [ações manuais](#acoes-manuais).

Não há CRUD, upload, autenticação, histórico de peso ou sincronização implementados. O contrato planejado não cria endpoints. Raça/espécie e localização/unidade ainda exigem integridade cruzada antes do cadastro funcional.
